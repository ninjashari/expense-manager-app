...
import { CategoryFormData, generateCategoryName } from "@/types/category"
...
export function CategoryForm({ category, onSubmit, onCancel, isLoading = false }: CategoryFormProps) {
  const [generatedName, setGeneratedName] = useState(category?.name || "")
   ...
   * @description Updates the generated name preview when display name changes
   * @param displayName - New display name value
   */
  const handleDisplayNameChange = (displayName: string) => {
    const generated = generateCategoryName(displayName)
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
            ...
            {generatedName && (
              <div className="space-y-2">
                <FormLabel>Backend Name (Auto-generated)</FormLabel>
                <div className="flex items-center space-x-2">
                  <Badge variant="secondary" className="font-mono">
                    {generatedName}
                  </Badge>
                  <FormDescription className="text-xs">
                    This unique identifier is automatically created from your display name.
                  </FormDescription>
                </div>
              </div>
            )}
